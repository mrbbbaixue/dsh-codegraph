 ...
 *
 * Queries ride a **resident MCP session** per project: one long-lived
 * `codegraph serve --mcp` child, which the launcher points at a detached shared
 * daemon. That daemon owns the file watcher and the single SQLite writer, so the
 * index is current by the time a query arrives and no query pays a process spawn.
 * Index writes that a person should authorize (`init`, `index`) and the one
 * command with no MCP counterpart (`affected`) stay on CLI subprocesses, which do
 * not contend for the daemon's writer lock.
 *
 * Three surfaces ride on that:
 *
 ...
 * - **B1** — codegraph's own `initialize.instructions` playbook as a
 *   system-prompt section, carried in full. Global scope, so subagents get it
 *   too — which is the audience the MCP `instructions` field never reaches.
...
export const Config = z.object({
  ...
  sessionIdleSec: z.natural().min(1).default(900),
})
...
/**
 * Refresh the index before a **CLI-path** query, and never let that refresh fail
 * the query.
 *
 * This is the fallback's freshness measure, not the normal one: a query on an
 * indexed project goes over the resident session, whose daemon watches the
 * project and syncs it within its debounce window. The CLI path has no watcher —
 * it runs when the project is unindexed or a tool has no MCP counterpart — and
 * there a symbol added since the last sync is silently invisible, which is
 * exactly the edit the model most often makes. Syncing first is what makes "trust
 * these results" a safe instruction; degrading to an annotated-but-successful
 * answer is what keeps a read-only project usable.
 *
 ...
 * codegraph's own output, returned verbatim.
 *
 * Two transports serve the model here. A tool with an MCP counterpart, on an
 * indexed project, goes over the resident session — no spawn, and the daemon's
 * watcher has already made the index current. Everything else (`index`,
 * `affected`, an unindexed project) runs the CLI once, where a pre-query `sync`
 * still carries the freshness the watcher would have provided.
 *
 ...
 *
 * @param deps - the plugin context and the live configuration thunk.
 * @param spec - name, description, parameters, timeout, CLI mapping, MCP mapping, and strictness.
...
function textTool(deps, spec) {
  return defineTool({
    ...
    async execute(args, exec) {
      ...
      // Only `explore` self-bootstraps: it is the entry point B1 names, so an
      // unindexed workspace must not turn that instruction into a dead end.
      const built = spec.name === EXPLORE_TOOL ? await deps.autoIndex(root, exec) : undefined

      if (spec.mcp !== undefined && isIndexed(root)) {
        return (built ?? '') + (await deps.sessions.call(root, spec.mcp, spec.call(args), exec.signal))
      }

      // A fresh build already reflects the disk; syncing on top of it buys nothing.
      const prefix = built ?? (spec.name === EXPLORE_TOOL ? await autoSync(deps, root, exec) : '')
...
function fullTools(deps) {
  ...
  return [
    textTool(deps, {
      ...
      mcp: 'codegraph_node',
      call: (args) => ({
        ...(args.name === undefined ? {} : { symbol: args.name }),
    ...
    textTool(deps, {
      ...
      mcp: 'codegraph_files',
      call: (args) => ({
        ...(args.filter === undefined ? {} : { path: args.filter }),
        ...(args.pattern === undefined ? {} : { pattern: args.pattern }),
        ...(args.format === undefined ? {} : { format: args.format }),
...
function assertUsableConfig(value) {
  for (const field of ['exploreTimeoutSec', 'indexTimeoutSec', 'sessionIdleSec']) {
 ...
 * @param ctx - the plugin context.
 * @param readConfig - thunk returning the currently authoritative configuration.
 * @param sessions - the resident MCP sessions, owned by {@link apply} so a
 *   settings change never tears down a live daemon connection.
 * @returns a function that rebuilds every derived registration.
 */
function createDerivedRegistrations(ctx, readConfig, sessions) {
  const owned = []
  const deps = { ctx, readConfig, sessions }
...
export function apply(ctx, entry) {
  ...
  // Created once, outside the rebuild: a settings change re-registers tools, and
  // must not cost the project its warm session and its daemon's watcher.
  const sessions = createSessions({ ctx, readConfig })
  const rebuild = createDerivedRegistrations(ctx, readConfig, sessions)
