 ...
 *
 * Plus one gate: `init`/`index` ask the user first, because rebuilding a whole
 * index is their call, not the model's.
 *
...
export const Config = z.object({
  ...
  exploreTimeoutMs: z.natural().min(1).default(120_000),
  indexTimeoutMs: z.natural().min(1).default(900_000),
})
...
Prefer this over grep/glob/read to locate or understand code whenever the workspace has a \`.codegraph/\` index at its root. A symbol added since the last index sync stays invisible until the next sync, which this plugin runs automatically before each query.

If the call reports that CodeGraph is not available here, the project is not indexed: say so and let the user decide. Do not index it yourself.`
...
function textTool(deps, spec) {
  return defineTool({
    ...
    async execute(args, exec) {
      ...
      const prefix = spec.name === EXPLORE_TOOL ? await autoSync(deps, root, exec) : ''
...
function coreTools(deps) {
  ...
  return [
    textTool(deps, {
      ...
      timeoutMs: config.exploreTimeoutMs,
      parameters: {
        query: {
          type: 'string',
          required: true,
...
function fullTools(deps) {
  const config = deps.readConfig()
  const budget = config.exploreTimeoutMs
  const pathArg = {
    type: 'string',
    description: 'Project directory. Defaults to the session workspace.',
  }
...
function assertUsableConfig(value) {
  for (const field of ['exploreTimeoutMs', 'indexTimeoutMs']) {
    if (!Number.isFinite(value[field]) || value[field] <= 0) {
      throw new Error(`${SETTINGS_NAMESPACE}: ${field} must be a positive number of milliseconds`)
    }
  }
}
